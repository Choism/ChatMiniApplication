...
public class LoginRequest extends AbstractRequest<NetworkResult<User>> {
    ...
    public LoginRequest(Context context, String email, String password, String regId) {
        HttpUrl url = getBaseUrlBuilder() //AbstrackRequst에 세팅해둔 부분
                .addPathSegment("signin") // 뒤에  요청 URl 뒤 세그먼트 부분 ex) 호스트 뒤 부분
                .build();

        //.build 해야 getBaseUrlBuilder+addPathSegment을 .Build 써줘야 붙여진다.
        ...
        RequestBody body = new FormBody.Builder() //서버에 저장된 key: value 을 세팅
                ...
                .build(); //post부분 : 서버랑 데이터를 어덯게 주고 받을지 세팅하는 하는 곳
